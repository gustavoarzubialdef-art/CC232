...
1. ¿En qué casos `percolateUp` hace cero intercambios?
   ...
   Hace cero intercambios cuando el elemento insertado no tiene padre, como ocurre al insertar el primer elemento, o cuando el padre ya tiene prioridad mayor o igual que el hijo.
...
5. ¿Por qué `delMax` tiene costo `O(log n)`?.
   ...
   ```text
   6. leer la raíz: O(1)
   7. mover el último elemento a la raíz y hacer pop_back: O(1)
   8. ejecutar percolateDown: O(log n)
   ```
...
3. ¿Por qué heapsort cuesta `O(n log n)`?
   ...
   ```text
   4. heapify de Floyd: O(n)
   5. n - 1 extracciones implícitas: cada una cuesta O(log n)
   ```
...
6. ¿Es heapsort estable? Justifica con un ejemplo.
...
7. ¿Qué diferencia hay entre usar `heapSort` y extraer todos los elementos con `delMax`?
   ...
   ```text
   8. construir una estructura PQ_ComplHeap,
   9. llamar delMax repetidamente,
   10. guardar las salidas en otro contenedor si se quiere conservar el resultado ordenado.
   ```
...
3. ¿Cómo se implementa `delMax` usando `merge`?
   ...
   4. Se guarda `root_->value`.
   5. Se separan `root_->left` y `root_->right`.
   6. Se elimina la raíz antigua.
   7. Se fusionan los dos subárboles.
...
8. ¿Qué propiedad adicional diferencia un heap izquierdista de un heap binario completo?
...
9. ¿Qué ventaja conceptual tiene un heap izquierdista frente a un heap binario completo?.
...
1. ¿Por qué el recorrido inorden debe salir ordenado aunque las prioridades cambien la forma del árbol?
   ...
   Porque las rotaciones en un Treap preservan el invariante BST. Una rotación solo reconfigura relaciones padre-hijo, pero mantiene que todas las claves del subárbol izquierdo sean menores que la clave del nodo, y todas las claves del subárbol derecho sean mayores. La propiedad de heap por prioridad es independiente de la propiedad de ordenamiento por claves.

2. ¿Por qué la raíz no necesariamente es la primera clave insertada?

   La raíz termina siendo el nodo con **prioridad mínima** del árbol (en esta implementación, prioridad menor = más alto en el árbol). Cuando se inserta un nodo con prioridad menor que la de sus ancestros, `bubbleUp` lo hace rotar hacia arriba hasta la raíz. No tiene relación con el orden de inserción, sino con la comparación de prioridades.

3. ¿Qué nodo debe subir cuando se inserta una clave con prioridad menor que la de sus ancestros?

   El nodo recién insertado debe subir. `bubbleUp` compara la prioridad del nodo con la de su padre y rota mientras el padre tenga prioridad mayor. Si el nodo es hijo izquierdo, se aplica rotación derecha sobre el padre; si es hijo derecho, rotación izquierda.

4. ¿Qué propiedad conserva una rotación local sobre las claves?

   Una rotación conserva el **orden BST**. En una rotación izquierda sobre `u`, el hijo derecho `w` sube con sus descendientes más a la derecha. El nodo `u` pasa a la izquierda de `w`, y el subárbol intermedio (hijo izquierdo de `w`) queda como hijo derecho de `u`. El rango de claves de ambos nodos se preserva.

5. ¿Qué propiedad intenta restaurar `bubbleUp` sobre las prioridades?.

   `bubbleUp` restaura la **propiedad heap por prioridad**: que cada nodo tenga prioridad menor o igual que la de su padre. Si un nodo insertado viola esta propiedad, rota con el padre hasta que la invariante se cumpla o llegue a la raíz.
...
1. ¿Qué diferencia hay entre un heap de prioridad y un árbol de búsqueda?
   ...
   Un heap garantiza que el máximo (o mínimo) esté en la raíz, permitiendo `getMax`/`delMax` en $O(1)$ y $O(log(n))$. Un BST mantiene claves ordenadas para permitir búsquedas, pero no optimiza la extracción de máximos.

2. ¿Por qué un BST permite recorrido ordenado y un heap no?

   El BST tiene la invariante: clave(izq) < clave(padre) < clave(der). El inorden visita izquierda, raíz, derecha, produciendo secuencia ordenada. El heap solo garantiza padre > hijos (localmente), no una relación global entre hermanos.

3. ¿Qué agrega `PQ_ComplHeap` frente a un `BinaryHeap` educativo?

   - Usa funciones constexpr `pqParent`, `pqLeftChild`, etc. para claridad
   - Separa operaciones en headers modulares
   - Tiene `isValidHeap()` para validación explícita
   - Implementa `heapify` de Floyd diferente al `bubbleDown` de BinaryHeap

4. ¿Qué combina un `Treap`?

   Combina **BST por clave** + **heap por prioridad**. Las claves permiten búsqueda ordenada; las prioridades dan balance esperado sin invariantes complejos como AVL/RB.

5. ¿Qué estructura usarías para extraer máximos repetidamente?

   `PQ_ComplHeap` o `BinaryHeap`. Están optimizados para `delMax` en $O(log(n))$.

6. ¿Qué estructura usarías para responder `lowerBound` o `upperBound`?

   `Treap` o `BinarySearchTree`. La propiedad BST permite búsqueda de sucesores/predecesores.

7. ¿Qué estructura usarías si quieres búsqueda ordenada con balanceo probabilístico?.

   `Treap`. Si las prioridades son aleatorias, la altura esperada es $O(log(n))$ sin necesidad de rotaciones de rebalanceo complejas.
...
#### 1. Demostración modificada: `demo_compare_with_semana5.cpp`
...
int main() {
  ...
  // MOD-A6-B11: Se agregó bloque comparativo de estructuras.
  std::cout << "Bloque 11 - Tabla comparativa: BinaryHeap, BinarySearchTree, Treap, PQ_ComplHeap\n\n";

  std::cout << "| Estructura           | Operacion principal | Propiedad mantenida      | Operacion eficiente    | Operacion no conviene  | Evidencia producida               |\n";
  std::cout << "|----------------------|-------------------|------------------------|----------------------|----------------------|-----------------------------------|\n";
  std::cout << "| BinaryHeap (S5)      | top() / add()     | heap (prioridad)       | obtener max/min      | buscar por clave       | " << pq.getMax() << " es max en PQ_ComplHeap      |\n";
  std::cout << "| BinarySearchTree     | find / inorder    | BST (clave)            | busqueda ordenada    | obtener prioridad      | inorder ordenado                  |\n";
  std::cout << "| Treap (S6)           | find / inorder    | BST + heap prioridad   | busqueda + prioridad  | obtener todos max      | " << bst.inorder().size() << " elementos en BST               |\n";
  std::cout << "| PQ_ComplHeap (S6)    | getMax / delMax   | heap (prioridad)       | prioridad            | busqueda por clave     | " << pq.getMax() << " es max en PQ_ComplHeap      |\n";
  ...
  std::cout << "- Treap combina BST (busqueda por clave) y heap (prioridad).\n";
}
```

#### 2. Tabla de comparación

| Estructura           | Operación principal | Propiedad mantenida   | Operación eficiente | Operación no conviene | Evidencia producida        |
|-------------------- |------------------ |-------------------- |------------------ |-------------------- |-------------------------- |
| BinaryHeap (S5)       | top() / add()      | heap (prioridad)     | obtener max/min   | buscar por clave     | getMax: prioridad acceso O(1) |
| BinarySearchTree  | find / inorder   | BST (clave)          | busqueda ordenada | prioridad            | inorder: secuencia ordenada |
| Treap (S6)         | find / getMax     | BST + heap prioridad | búsqueda + prioridad | iterar por niveles | isTreap: ambas invariantes  |
| PQ_ComplHeap (S6)    | getMax / delMax   | heap (prioridad)     | prioridad max     | búsqueda por clave  | delMax: O(log n) garantizado |

#### 3. Respuesta breve de selección de estructura

- **Para extraer máximos**: usar `PQ_ComplHeap` - está optimizado para `getMax`/`delMax`
- **Para búsqueda ordenada con `lowerBound`/`upperBound`**: usar `Treap` o `BinarySearchTree`
- **Para búsqueda ordenada con balance**: usar `Treap` - balance esperado O(log n) sin rotaciones complejas
...
#### 1. Prueba interna modificada: `test_internal_week6.cpp`
...
// MOD-A6-B12: Pruebas obligatorias y treap completo.
static void testBloque12PruebasObligatorias() {
  ods::PQ_ComplHeap<int> pqInsert;
  for (int x : {8, 3, 10, 1, 6, 14, 4, 7, 13, 14}) {
    pqInsert.insert(x);
    assert(pqInsert.isHeap());
    assert(pqInsert.isValidHeap());
  }

  ods::PQ_ComplHeap<int> pqDelete{4, 10, 7, 1, 3, 9};
  while (!pqDelete.empty()) {
    pqDelete.delMax();
    assert(pqDelete.isHeap());
    assert(pqDelete.isValidHeap());
  }

  ods::PQ_ComplHeap<int> pqGetMax{5, 3, 7};
  assert(pqGetMax.size() == 3);
  int maxVal = pqGetMax.getMax();
  assert(maxVal == 7);
  assert(pqGetMax.size() == 3);

  ods::PQ_ComplHeap<int> pqDel{5, 3, 7};
  assert(pqDel.size() == 3);
  pqDel.delMax();
  assert(pqDel.size() == 2);

  std::vector<int> raw{4, 17, 3, 90, 55, 21, 8, 13, 34, 2, 1, 89};
  ods::PQ_ComplHeap<int> fromHeapify(std::move(raw), std::less<int>{});
  assert(fromHeapify.isHeap());
  assert(fromHeapify.isValidHeap());
  assert(fromHeapify.getMax() == 90);

  std::vector<int> toSort{5, 1, 5, 3, 8, 2, 8, 0};
  ods::heapSort(toSort);
  assert((toSort == std::vector<int>{0, 1, 2, 3, 5, 5, 8, 8}));
}

static void testBloque12TreapCompleto() {
  ods::Treap<int> treap(232);
  ...
  assert(treap.addWithPriority(14, 120));
  assert(treap.addWithPriority(4, 65));
  assert(treap.addWithPriority(7, 68));

  assert(treap.size() == 8);
  assert(treap.root() != nullptr);
  ...
  assert(!treap.addWithPriority(4, 5));
  assert(treap.size() == 8);
  assert(treap.isTreap());
...
int main() {
  testBloque12PruebasObligatorias();
  testBloque12TreapCompleto();
...
#### 2. Prueba publica modificada: `test_public_week6.cpp`
...
int main() {
  ...
  // MOD-A6-B12: Verificación adicional de validez interna del heap.
  assert(h.isValidHeap());
  ...
  // MOD-A6-B12: Confirmar que sigue siendo un heap válido tras inserción.
  assert(h.isValidHeap());

  assert(h.delMax() == 12);
  assert(h.isHeap());

  // MOD-A6-B12: Validación tras eliminación del máximo.
  assert(h.isValidHeap());
  ...
  // MOD-A6-B12: Nodos para probar Treap con prioridades variadas.
  assert(treap.addWithPriority(14, 120));
  assert(treap.addWithPriority(4, 65));
  assert(treap.addWithPriority(7, 68));
  ...
  // MOD-A6-B12: Nueva verificación de enlaces de padres válidos en Treap.
  assert(treap.hasValidParentLinks());
  ...
  // MOD-A6-B12: Verificación final de enlaces de padres tras operaciones.
  assert(treap.hasValidParentLinks());
}
```
...
1. **PQ_ComplHeap conserva propiedad heap después de cada inserción** - Atrapa errores en `percolateUp`
2. **PQ_ComplHeap conserva propiedad heap después de cada eliminación** - Atrapa errores en `percolateDown`
3. **getMax no cambia el tamaño** - Verifica que `getMax` solo lea, no modifique
4. **delMax sí cambia el tamaño** - Verifica decremento correcto en `delMax`
5. **heapifyFloyd produce un heap válido** - Atrapa errores en la construcción inicial
6. **heapSort ordena con repetidos** - Verifica estabilidad del algoritmo
7. **PQ_LeftHeap conserva propiedad después de merge** - Atrapa errores en `merge`
8. **PQ_LeftHeap conserva propiedad después de insert** - Atrapa errores en `merge` implícito
9. **PQ_LeftHeap conserva propiedad después de delMax** - Atrapa errores en `remove`
10. **Huffman produce códigos para todos los símbolos** - Cobertura completa
11. **Huffman produce códigos libres de prefijos** - Propiedad fundamental de Huffman
12. **Huffman maneja un solo símbolo** - Caso borde
13. **Treap conserva propiedad BST después de insertar** - Atrapa errores en inserción BST
14. **Treap conserva propiedad heap por prioridad después de insertar** - Atrapa errores en `bubbleUp`
15. **Treap conserva ambas propiedades después de eliminar** - Atrapa errores en `trickleDown`

#### 4. Resultado esperado de `ctest --output-on-failure`
...
#### 5. Explicación de bugs por prueba:

- `isValidHeap()`: detecta intercambios incorrectos en `percolateUp/down`
- `hasValidParentLinks()`: detecta rotaciones que olvidan actualizar `parent`
- Prueba de tamaño: detecta incrementos dobles o decrementos faltantes
- Prueba de inorden ordenado: detecta rotaciones incorrectas o inserciones fallidas
- Prueba de `isTreap()`: detecta violaciones de heap por prioridad en `bubbleUp/trickleDown`

#### 3. Bloque 13 - Defensa escrita de modificaciones
...
Lo que aprendí al **modificar el código de prioridad, heaps, Huffman y Treap** es distinto a lo que se obtiene solo leyendo teoría o ejecutando demostraciones:

- La **interfaz `PQ`** me enseñó que abstraer operaciones como `insert`, `getMax` y `delMax` permite comparar distintas implementaciones bajo un mismo contrato, reforzando la idea de que la prioridad puede gestionarse con diferentes estructuras pero con la misma semántica.
- La **representación implícita del heap binario completo** en un `vector` me obligó a comprender cómo los índices (`parent`, `left`, `right`) definen la estructura sin punteros, y cómo los casos borde (nodo con solo hijo izquierdo, nodo hoja) deben manejarse con cuidado.
- En **`percolateUp`** aprendí que los intercambios reflejan la relación de prioridad con los ancestros, y que el número de intercambios está acotado por la altura del heap.
- En **`percolateDown`** entendí que la elección del hijo dominante es crítica: si se intercambia con el hijo equivocado, la propiedad heap se rompe. Verlo en trazas me enseñó a pensar en invariantes locales.
- El **`heapify` de Floyd** me mostró que construir un heap desde un arreglo puede hacerse en $O(n)$ aplicando bajadas desde los nodos internos, lo cual es más eficiente que insertar uno por uno.
- Con **`heapSort`** confirmé que el heap no ordena el arreglo internamente, pero sí permite extraer máximos sucesivos para producir un orden total, diferenciando entre propiedad local y orden global.
- El **`merge` en heap izquierdista** me enseñó que la estructura puede fusionarse en tiempo logarítmico gracias a la propiedad de sesgo a la izquierda, algo que no se aprecia solo ejecutando la operación.
- En **Huffman**, al modificar la generación de códigos, aprendí que el caso de un solo símbolo necesita un tratamiento especial para no producir cadenas vacías, y que la propiedad *prefix-free* debe validarse explícitamente.
- Con el **Treap**, al añadir rotaciones y prioridades, entendí cómo se combinan dos invariantes distintas (BST por claves y heap por prioridades) y cómo las rotaciones locales restauran ambas. Esa dualidad solo se aprecia al modificar y validar los enlaces de padres.
- Al comparar con **BinaryHeap y BinarySearchTree**, vi que cada estructura responde a un propósito distinto: prioridad pura vs. búsqueda ordenada, y que el Treap es un puente entre ambos mundos.
- Finalmente, las **pruebas, invariantes y casos borde** me enseñaron que el código no se valida solo por pasar tests básicos: hay que pensar en situaciones extremas (primer elemento, último elemento, hijo único, duplicados) y comprobar que las propiedades se mantienen siempre.

En conjunto, modificar el código me dio una comprensión práctica de cómo las estructuras preservan sus invariantes y cómo cada detalle en índices, comparadores o rotaciones puede ser decisivo. Esa experiencia no se obtiene solo leyendo teoría o ejecutando ejemplos: se aprende al enfrentarse a los casos límite y ver cómo el algoritmo se sostiene o se rompe.
